  ...
  const routes = [
    ...
    // POST /api/dsh-mobilecode/connect {host, port?, pairing_code?, pairing_port?} — attach a Wi-Fi device (with optional pairing).
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const connected = await DeviceBuild.adbConnect(target)
    ...
    // POST /api/dsh-mobilecode/pair-qr {timeout_ms?} — WIFI:T:ADB QR + mDNS auto-pair + connect (one blocking call).
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const connected = await DeviceBuild.adbConnect(`${connectHost}:5555`)
    ...
    },
    {
      ...
      handler: async (req, res) => {
        if (!refGuard(req, res)) return
        if (!isPost(req, res)) return
        const body = await readBody(req, res, 24 * 1024 * 1024)
        if (body === undefined) return
        // The server never pixel-decodes: the client canvas decodes both images,
        // downsamples them IDENTICALLY, and posts raw RGBA. Encoded images are
        // refused with the reason, never approximated (Feature E honesty rule).
        const frameOf = (f) => {
        ...
        if (a === undefined || b === undefined) {
          writeJson(res, 400, {
            error: 'frames must be {width, height, rgba:base64} with exact byte counts — decode the images in the browser canvas, downsample both to the SAME size, then post',
            note: 'the server deliberately does not decode PNG/JPEG (no image dependency); encoded images are refused, not approximated',
          })
          return
        }
        const mode = body?.mode === 'design-reference' ? 'design-reference' : 'regression'
        try {
          const diff = VisualCompare.diffFrames(a, b, { threshold: Number(body?.threshold) || 24, masks: Array.isArray(body?.masks) ? body.masks : [] })
          const envelope = VisualCompare.compareEnvelope({
            ...
            env: { downsampledBy: 'client-canvas', frameBytes: a.rgba.length },
            limitations: ['frames are client-downsampled — region rects are in downsampled pixel space, not device pixels'],
          })
          try {
    ...
    // ── Compose preview gallery (Feature B/T5) ────────────────────────────────
    {
      ...
      handler: async (req, res) => {
        ...
        const warnings = []
        const entries = gradle ? PreviewGallery.discoverPreviewTests(directory, warnings).map((e) => ({ ...e, ...PreviewGallery.renderState(directory, e) })) : []
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        const entry = PreviewGallery.discoverPreviewTests(directory).find((e) => e.fqClass === className && e.method === method)
        const state = entry ? PreviewGallery.renderState(directory, entry) : undefined
        ...
        try {
          ...
          res.writeHead(200, { 'content-type': 'image/png', 'cache-control': 'no-store', 'x-content-type-options': 'nosniff', 'x-preview-stale': state.stale ? 'true' : 'false' })
          res.end(bytes)
        } catch { writeJson(res, 500, { error: 'failed to read image' }) }
      },
    },
    ...
    // ── OpenPencil narrow adapter (T7/Feature D — optional, degrades honestly) ─
    {
      ...
      handler: async (req, res) => {
        ...
        writeJson(res, 200, {
          ...
          binary: p.resolved,
          hint: p.installed ? 'file-mode CLI adapter active: inspect .fig docs, export a frame to PNG and import it as a reference, read design variables.' : 'install OpenPencil (openpencil.dev — releases/Homebrew; Windows via npm or release binary) to enable frame export; the reference workspace stays fully usable with manual image imports.',
  ...
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      const previews = entries.map((e) => {
        const s = PreviewGallery.renderState(directory, e)
        return { class: e.fqClass, method: e.method, previewName: e.previewName, widthDp: e.widthDp, sourceFile: path.relative(directory, e.sourceFile).replaceAll('\\', '/'), rendered: s.rendered, stale: s.stale }
      })
      return { status: 'passed', count: previews.length, previews, ...(discoveryWarnings.length ? { warnings: discoveryWarnings } : {}), limitations: ['stale = source newer than the recorded PNG; re-render before trusting it', 'host-rendered previews are not device verification'] }
    },
  })
}
