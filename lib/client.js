...
`;
		...
		function RefCompareSection({ directory }) {
			...
			const decodeDownsample = async (src, maxDim = 480) => {
				const img = new Image();
				...
				return { width: w, height: hgt, rgba: ctx2d.getImageData(0, 0, w, hgt).data };
			};
			const runDiff = async () => {
				if (!refSrc || !capSrc) { setError("need both a reference and a capture to diff"); return; }
				await run("diff", async () => {
					const [a, b] = await Promise.all([decodeDownsample(refSrc), decodeDownsample(capSrc)]);
					if (a.width !== b.width || a.height !== b.height) throw new Error("downsampled sizes differ (" + a.width + "x" + a.height + " vs " + b.width + "x" + b.height + ") — reference and capture aspect ratios differ; align first");
					...
					const r = await streamPost("/refs/diff", {
						...
						mode: "design-reference", project, screen, stateId,
			...
			return h("div", { className: "mc-card", style: { borderTop: "1px solid var(--mc-border, #2a3340)" } },
				...
				h("div", { className: "mc-row" },
					...
					h("button", { className: "mc-btn", disabled: busy !== "" || !refSrc || !capSrc, title: "server-side cluster diff on downsampled frames — evidence file persisted", onClick: runDiff }, "Diff"),
				),
				...
				diff && h("div", { style: { border: "1px solid var(--mc-border, #2a3340)", borderRadius: 8, padding: 8, fontSize: 11 } },
					h("div", null, h("b", null, "diff · " + diff.mode + " · " + diff.checkStatus),
						h("span", { style: { opacity: 0.7 } }, "  " + diff.diff.changePct + "% changed (" + diff.diff.changedPixels + "/" + diff.diff.totalPixels + " px @ " + diff.diff.status + ")")),
					(diff.diff.clusters ?? []).slice(0, 6).map((c) => h("div", { key: c.x + "," + c.y, style: { opacity: 0.85 } },
						"region " + c.x + "," + c.y + " " + c.width + "×" + c.height + " (" + c.pixels + " px)")),
			...
			return h("div", { className: "mc-card" },
				...
				entries.length > 0 && h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 8 } },
					entries.map((entry) => h("div", { key: entry.fqClass + "." + entry.method, style: { border: "1px solid var(--mc-border, #2a3340)", borderRadius: 8, padding: 6, fontSize: 11 } },
						...
						entry.stale && entry.rendered && h("span", { style: { color: "#ffb020" } }, "stale — source changed"),
